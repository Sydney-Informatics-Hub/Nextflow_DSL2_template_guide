...
Consider processes as individual units of work or steps to be executed by a workflow. While you can opt to define all processes within the `main.nf` script (like [here](https://github.com/nextflow-io/rnaseq-nf/blob/3b5b49f/main.nf)), we have chosen to focus on modularity with this template. As such, processes inside the template are saved as individual scripts inside the `modules/` directory and [imported](https://training.nextflow.io/basic_training/modules/#importing-modules) into the `main.nf` script: 
